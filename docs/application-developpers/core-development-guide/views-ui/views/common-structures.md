...
Some attributes are meant to specific view types. The lists below recaps the attributes specific to each view type.
...
The `routes` property allows defining contextual links to other parts of the application. These links are displayed in the view and can redirect the user to a specific context or view based on the configuration.
...
| label        | `string`       | The label displayed to the user for the route.                                 |
| description  | `string`       | A short description of the route's purpose.                                    |
| view         | `string`       | The ID of the target view to display when the route is followed.               |
| params       | `array`        | (optional) Associative array mapping fields to values passed to the target view.|
| visible      | `array>domain` | (optional) Domain conditions to determine if the route should be displayed.    |

### `access`

Associative map to restrict the access of the view to specific users.
...
Each entry of the `operations` object associates a name (ID of an operation - which will allow to group the results), with an associative array mapping field names with operation descriptors.
