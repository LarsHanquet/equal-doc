...
!!! note 
    As the only condition for the SQL type is to be compatible with the associated eQual type, in order for a binary field to be translated (for instance a PDF doc available in different languages), the size of the PDF should remain smaller than the SQL MEDIUMBLOB type (16 MB with MySQL).
...
Using the `$lang` parameter (`DEFAULT_LANG` by default) indicates to the ORM which value to return for fields set as `multilang`.

> You don't need to specify language for a **DELETE** request since the whole object gets deleted
...
It updates your initial object with the secondary language.
...
??? example "Example of french (fr) translation using i18n"
    ```json
    {
    "name": "Règle de Tva",
