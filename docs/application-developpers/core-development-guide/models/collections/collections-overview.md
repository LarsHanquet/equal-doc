...
In eQual, a collection is a series of partial objects (the ORM loads only requested fields) on which one or more [CRUD operations](TODO) can be applied. Using collections enables method chaining, making code in controllers shorter and easier to read.
...
For more details on the ORM and its integration with collections, see [ORM Documentation](TODO).
