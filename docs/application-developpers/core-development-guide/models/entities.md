...
The models defintions are located in the `/packages/{package_name}/classes` folder of the package they relate to (see #TODO [Directory Structure](/architecture-concepts/directory-structure)).   
...
!!! tip Action controller `core_test_package-consistency` can help to spot any incompatibility or inconsistency in the definition of classes from a given package.
...
!!! tip "usage property"
	The `usage` property is complementary to the `type` property and allows to refine the way the field should be handled by the ORM and the DBMS, and how it should be rendered in the UI (see #TODO[Usages](/architecture-concepts/usages/)). 
...
By default, ints are stored in DBMS using `INT(11)`.
...
By default, floats are stored in DBMS using `DECIMAL(10,2)`.
...
Once the computed value is evaluated (either via `function` or `relation`), and if `store` is set to `true`, the result will be **persisted in the database**.
...
| ---------------|----------| ------------------------------------------------------------ |
| type        | one of the value listed as <a href="#definition_field_types">fields types</a> |  The type of field. |
| usage       | `string` | Specifies additional information about the format of the field (@see #TODO [Usages](../architecture-concepts/usages.md) for more details) |
| description | `string` | Brief about the field (max 65 chars).              |
| visible     | (optional) `boolean` \| `array` | #TODO [Domain](../architecture-concepts/domains.md) holding the conditions that must be met in order for the field to be relevant (and shown in UI). |
...
| multilang   | `boolean` | Marks the field as [translatable](../../i18n/) (default = false). |
...
| domain  | (only relational fields) `array` | #TODO [Domain](../architecture-concepts/domains.md) holding the additional conditions to apply on the set of objects targeted by the relation. |
...
|              | multilang | (optional, `boolean`) Tells if field is [translatable](../../i18n/) (default = false) |
...
|              | multilang | (optional, `boolean`) Flag telling if field can be [translated](../../i18n/) (default value: false). This applies only for stored fields. |
...
| status   | `string `        | 'draft', 'instance' or 'archive'.                              |
